    ...
    client.once("ready", async () => {
                    ...
                    if (command.guildID && command.guildID.length > 0) {
                        for (const guildId of command.guildID) {
                            ...
                            try {
                                ...
                                log(`SUCCÈS : Commande "${command.name}" déployée sur la guilde ${guildId}`);
                            } catch (err: any) {
                        ...
                        log(`AJOUT : Commande "${command.name}" ajouté à la file d'attente`);
        ...
        if (commandArray.length > 0) {
            try {
                ...
                log(`SUCCÈS : ${numberCommandDeployed}/${fileLength} commandes globales déployées`);
            } catch (err: any) {
                log(`ERREUR CRITIQUE : Déploiement des commandes globales : ${err.message}`);
            }
        }

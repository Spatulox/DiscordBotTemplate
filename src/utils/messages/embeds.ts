...
export function createErrorEmbed(description: string): Embed{
...
export async function sendInteractionEmbed(interaction: CommandInteraction | ModalSubmitInteraction | BaseInteraction, embed: Embed, privateVisibility: boolean = false): Promise<boolean> {
    ...
    try {
        ...
        console.log(`INFO : Embed '${embed?.title || embed?.description || 'sans titre :/'}' envoyé à l'utilisateur via l'interaction '${interaction.id}'`);
        return true;
    } catch (e) {
        console.error(
